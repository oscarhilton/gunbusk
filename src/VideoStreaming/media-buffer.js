...
function Mediabuffer(
	...
	this.disableMobileCheck =
		typeof disableMobileCheck !== "undefined" ? disableMobileCheck : false;
	this.forceFullDownload =
		typeof forceFullDownload !== "undefined" ? forceFullDownload : false;
...
Mediabuffer.prototype.getAverageDelay = function () {
	...
	if (this.delays.length) {
		sum = this.delays.reduce(function (a, b) {
			return a + b;
		});
...
Mediabuffer.prototype.progress = function () {
	...
	if (numberOfTimeRangesLoaded > 0) {
		...
		if (
			secondsToLoad === 0 ||
			(elapsedTime > 0 &&
				secondsLoaded > estimatedRemainingDownloadSeconds &&
				!this.forceFullDownload)
		) {
			this.destroy(false);
			this.readyCallback();
			...
			if (elapsedTime > 0) {
				this.percentBuffered = this.forceFullDownload
					? Math.round((secondsLoaded / duration) * 100)
					: Math.round(
							(secondsLoaded / estimatedRemainingDownloadSeconds) * 100
					  );
